...
-- Found at https://devforum.roblox.com/t/vector3fuzzyeq-and-its-alias-isclose-return-incorrect-results/726222/2
...
function vector3:FuzzyEq(v3, epsilon)
	local isVector = v3.__type and v3.__type == "vector3"
	if (isVector) then
		return fuzzyEqVec(self, v3, epsilon or 1e-5)
	else
		error("bad argument #1 to 'FuzzyEq' (Vector3 expected, got number)")
	end
end

function vector3:Angle(v3)
local isVector = v3.__type and v3.__type == "vector3"
	if (isVector) then
		return math.acos(math.max(-1, math.min(1, self:Dot(v3) / self.magnitude / v3.magnitude)))
	else
		error("bad argument #1 to 'Angle' (Vector3 expected, got number)")
	end
end
